function search(matchValue) {
	...
	if (searchStr.indexOf('*') > -1 || searchStr.indexOf('x') > -1) {
		searchStr = searchStr.split('*').join('¡Á').split('x').join('¡Á')
	}
	...
	for (var t = 0; t < tableList.length; t++) {
		...
		for (var g = 0; g < groupList.length; g++) {
			...
			if ($currentGroup.querySelectorAll(".table-row:not(.hide)").length == 0) {
				...
				if ($currentGroupHeader.classList.contains("hide") && $currentGroupContent[0].querySelectorAll(".table-cell")[0].innerHTML.indexOf("»ØºÏ") == -1) {
