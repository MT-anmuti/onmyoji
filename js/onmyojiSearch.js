		...
		if (matchResult.result) {
			...
			for (var ma = 0; ma < matchResult.matchArray.length; ma++) {
				var matchStr = matchResult.matchArray[ma],
					lightHtml = "<span class='lightRed'>" + matchStr + "</span>";
				$currentCell.innerHTML = innerHTML.split(matchStr).join(lightHtml);
			}
		} else if ($currentCell.querySelectorAll("span.lightRed").length == 0) {
...
function resetLightFont() {
	var lights = document.querySelectorAll("span.lightRed");
